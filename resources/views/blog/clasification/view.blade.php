        ...
        <div class="row">
            <div class="p-4 fs-5">
                ...
                <div class="rounded p-4">
                            ...
                            <div>
                                <div class="row g-0 justify-content-center">
                                    <div class="card mb-4 shadow " style="max-width: 740px; border:none;">
                                        <div class="row g-0">
                                            <div class="col-md-4">
                                                <img src="{{ asset('storage/' . $entrada_blog->cover) }}"
                                                    class="img-fluid rounded-start"
                                                    alt="{{ $entrada_blog->cover_description }}">
                                            </div>
                                            <div class="col-md-8">
                                                <div class="card-body">
                                                    <h3 class="card-title border-bottom pb-2">{{ $entrada_blog->title }}
                                                    </h3>
                                                    <p class="card-text">{{ $entrada_blog->recortar_descripcion() }}</p>
                                                    <p><small
                                                            class="bg-verde badge rounded-pill text-light">{{ $entrada_blog->clasification->name }}</small>
                                                    </p>
                                                    <p><small>Publicado el {{ $entrada_blog->updated_at }}</small></p>
                                                </div>
                                            </div>
                                            <div class="d-flex justify-content-end pb-3">
                                                <a href="{{ route('blog.view', ['id' => $entrada_blog->blog_id]) }}"
                                                    class="link me-4">Seguir leyendo »</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @if ($entradas_count === 0)
                        <div>
                            <p class="text-center">No existe ninguna entrada con esa clasificación.</p>
                        </div>
                    @endif
                </div>
            </div>
